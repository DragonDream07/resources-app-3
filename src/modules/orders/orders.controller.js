...
async function getOrder(req, res, next) {
  try {
    ...
    const userId = req.user.id;
    const isAdmin = req.user.role === 'admin';
    const order = await ordersService.getOrder(orderId, userId, isAdmin);
...
async function advanceOrder(req, res, next) {
  try {
    ...
    const order = await ordersService.advanceOrder(orderId, status, { trackingNumber, trackingUrl, carrier });
    return res.status(200).json(order);
  } catch (err) {
    next(err);
  }
...
async function createReturnRequest(req, res, next) {
  try {
    ...
    const returnRequest = await ordersService.createReturnRequest(orderId, userId, { reason, items, notes });
